...
function parseXlsxData(fpath, sheetName, colNone = 0) {
  ...
  let refParse = ref.match(/[a-z]+|[0-9]+/gi);
...
function autoFill(point, index, mergesParses, sheetData) {
  ...
  // 返回合并单元格左上单元格数据
  result =
    (sheetData[String.fromCharCode(result[1] + 64) + result[0]] || {}).w || "";
  return result;
}
